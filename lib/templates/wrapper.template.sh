...
if [ -f "$ENV_FILE" ]; then
    ...
    # Reject files with dangerous characters that could enable command injection
    # Only allow: alphanumeric, underscore, hyphen, dot in KEY; safe chars in VALUE
    if grep -qE '^[A-Za-z_][A-Za-z0-9_.-]*=[^=]*$' "$ENV_FILE" && \
