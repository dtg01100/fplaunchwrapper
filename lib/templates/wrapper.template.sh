...
# Strict mode: errors must not silently fall through to flatpak run with attacker-controlled args.
# ``set -u`` is intentionally omitted because the template references ``$1``,
# ``$2``, ``$FPWRAPPER_HOOK_FAILURE``, etc. before the user invokes the wrapper;
# enforcing ``set -u`` would make those references blow up on a no-arg call.
set -eo pipefail
IFS=$'\n\t'
umask 077
# NOTE: This file contains Python template escapes ({{ and }}) that become single
# braces after .format() substitution. shellcheck sees {{ as invalid but it's
# intentional for Python string formatting.
