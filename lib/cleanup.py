...
class WrapperCleanup:
    ...
    def confirm_cleanup(self) -> bool:
        ...
        self.log(f"- Bash completion:   ~/.bashrc.d/fplaunch_completion.bash")
        self.log(f"- Man pages:         ~/.local/share/man")
        self.log("")
    ...
    def _cleanup_cron_entries(self):
        """Remove cron entries"""
        if not self.cleanup_items["cron_entries"]:
            return

        if self._command_available("crontab"):
    ...
    def _cleanup_wrappers_and_scripts(self):
    ...
    def _cleanup_completion_files(self):
        """Remove bash completion files"""
        for completion_file in self.cleanup_items["completion_files"]:
            self._remove_file(
                completion_file, f"Removing completion file: {completion_file}"
            )

    def _cleanup_man_pages(self):
        """Remove man pages"""
    ...
    def _cleanup_config_dir(self):
        ...
        if self.cleanup_items["config_dir"]:
            ...
            self._remove_directory(
                config_dir, f"Removing config directory: {config_dir}"
            )

    def _remove_file(self, path: Path, description: str):
    ...
    def _remove_directory(self, path: Path, description: str):
