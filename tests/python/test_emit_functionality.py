...
@pytest.mark.skipif(
    not GENERATE_AVAILABLE,
    reason="WrapperGenerator tests disabled when WrapperGenerator not available",
)
...
class TestEmitVerboseContent:
    ...
    def test_generate_emit_verbose(self) -> None:
        ...
        content = WrapperGenerator("/tmp/test", emit_mode=True, emit_verbose=True).create_wrapper_script(
            "firefox",
            "org.mozilla.firefox",
        )
