...
old_method = content[content.find('    def create_wrapper_script'):content.rfind('"""', 0, content.find('    def generate_all_wrappers')) + 3]
...
for var in variables:
    if var not in python_vars:
        bash_script = bash_script.replace(f'${{{var}}}', f'${{{{var}}}}')
